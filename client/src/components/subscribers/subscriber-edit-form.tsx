...
export function SubscriberEditForm({ subscriber, open, onOpenChange, onSuccess }: SubscriberEditFormProps) {
  ...
  const form = useForm<z.infer<typeof subscriberEditSchema>>({
  ...
  // Preencher o formulário quando o assinante mudar
  useEffect(() => {
    if (subscriber) {
      form.reset({
  ...
  const onSubmit = async (values: z.infer<typeof subscriberEditSchema>) => {
